...
const SUBJECTS = [
  { id: 'mathematics' as Subject, label: 'Maths',     icon: '📐', color: '#7C3AED' },
  { id: 'physics'     as Subject, label: 'Physics',   icon: '⚡', color: '#2563EB' },
  { id: 'chemistry'   as Subject, label: 'Chemistry', icon: '⚗️', color: '#059669' },
  { id: 'biology'     as Subject, label: 'Biology',   icon: '🧬', color: '#EA580C' },
  { id: 'general'     as Subject, label: 'Other',     icon: '🔬', color: '#6B7280' },
]
...
export default function SolvePage() {
  ...
  return (
    <div className="flex h-full overflow-hidden bg-paper">
      ...
      {/* ══ LEFT — Input panel ══════════════════════════════════════════════ */}
      <div className="w-[400px] flex-shrink-0 border-r border-line flex flex-col overflow-hidden bg-white">

        {/* Header */}
        <div className="px-5 py-4 border-b border-line">
          <h1 className="text-lg font-display font-extrabold tracking-tight text-ink">Ask a question</h1>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          {/* ── Subject picker ─────────────────────────────────────────── */}
          <div>
            <p className="text-xs text-muted font-mono mb-2">What subject is this?</p>
              ...
              {SUBJECTS.map(s => (
                <button key={s.id} onClick={() => setSubject(s.id)}
                  ...
                  style={subject === s.id
                    ...
                    : { borderColor: '#E5E5E0', color: '#6B7280' }
                  }>
                  {s.icon} {s.label}
                </button>
              ))}
          ...
          <div className="relative">
            <textarea
              ...
              className="w-full bg-paper border border-line rounded-xl px-4 py-3 text-sm text-ink placeholder-muted focus:outline-none focus:border-primary/40 focus:ring-1 focus:ring-primary/20 transition-colors resize-none disabled:opacity-40 disabled:cursor-not-allowed"
            />
            {text && (
              <button onClick={() => setText('')}
                className="absolute top-2.5 right-2.5 text-muted/40 hover:text-muted transition-colors text-xs">
                ✕
              </button>
            )}
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-muted font-mono">or upload a photo</span>
            <div className="flex-1 h-px bg-line" />
          </div>
              ...
              <div {...getRootProps()}
                className={`border-2 border-dashed rounded-xl p-5 text-center cursor-pointer transition-all ${
                  isDragActive
                    ...
                    : 'border-line hover:border-muted/40 hover:bg-paper'
                }`}>
                <input {...getInputProps()} />
                <div className="text-3xl mb-2">📸</div>
                <p className="text-sm font-semibold text-ink/70 mb-0.5">
              ...
              <div className="relative">
                {preview ? (
                  <img src={preview} alt="Your question"
                    className="w-full max-h-48 object-contain rounded-xl border border-line bg-paper" />
                ) : (
                  <div className="flex items-center gap-3 p-4 bg-paper border border-line rounded-xl">
                    <span className="text-2xl">📄</span>
                    <span className="text-sm text-ink/70 font-mono truncate">{file.name}</span>
                  </div>
                )}
                <button onClick={clearInput}
                  className="absolute top-2 right-2 w-7 h-7 rounded-full bg-white border border-line text-muted hover:text-ink flex items-center justify-center text-xs transition-colors shadow-sm">
                  ✕
                </button>
                <p className="text-xs text-muted font-mono text-center mt-2">Photo ready ✓</p>
              </div>
            )}
          </div>
          ...
          <AnimatePresence>
            {showExamples && (
              <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }}>
                <p className="text-xs text-muted font-mono mb-2">Try an example:</p>
                <div className="space-y-2">
                  {EXAMPLES[subject].map((ex, i) => (
                    <motion.button key={ex} onClick={() => handleExampleClick(ex)}
                      className="w-full text-left text-xs text-ink/60 bg-paper border border-line rounded-lg px-3 py-2.5 hover:border-muted/30 hover:text-ink/80 hover:bg-white transition-all"
        ...
        <div className="p-4 border-t border-line">
          ...
          {(text.trim() || file) && state !== 'solving' && (
            <p className="text-center text-[10px] text-muted font-mono mt-2">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {/* Solution topbar */}
        <div className="flex items-center justify-between px-7 py-3.5 border-b border-line flex-shrink-0 bg-white">
          <div className="flex items-center gap-3">
            <span className="text-xl">{activeSubject.icon}</span>
            <div>
              <div className="text-sm font-display font-bold" style={{ color: activeSubject.color }}>
          ...
          <div className="flex items-center gap-2">
            ...
            {state === 'done' && (
              <motion.button
                ...
                className="text-[11px] font-mono text-muted hover:text-ink border border-line px-3 py-1.5 rounded-lg transition-colors hover:bg-paper"
        ...
        <div ref={solutionRef} className="flex-1 overflow-y-auto px-7 py-6">
          ...
          {state === 'idle' && (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-base font-display font-bold text-muted mb-2">Your solution will appear here</p>
              <p className="text-sm text-muted/60 font-mono max-w-xs leading-relaxed">
          ...
          {state === 'solving' && !solution && (
            <div className="flex flex-col items-center justify-center h-full gap-5">
              ...
              <div className="text-center">
                <p className="text-base font-display font-semibold text-ink/70 mb-1">Working it out...</p>
                <p className="text-xs text-muted font-mono">AI is reading your question</p>
              </div>
              {/* Animated dots */}
            ...
            <div className="max-w-3xl">
                ...
                <motion.div
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(metadata?.keyFormulas ?? []).map((f: string) => (
                      <div key={f}
                        className="px-3 py-2 rounded-lg text-sm font-mono border bg-white"
                        style={{ borderColor: `${activeSubject.color}20`, color: activeSubject.color }}>
                        {f}
                      </div>
                    ))}
                  </div>
              ...
              {state === 'done' && (
                <motion.div
                  ...
                  <span className="text-muted font-mono text-xs">Was this helpful?</span>
                  {['👍', '👎'].map(emoji => (
                    <button key={emoji}
          ...
          {state === 'error' && !solution && (
            <div className="flex flex-col items-center justify-center h-full text-center gap-4">
              ...
              <div>
                <p className="text-base font-display font-bold text-ink/60 mb-1">Something went wrong</p>
                <p className="text-sm text-muted font-mono">Check your connection and try again</p>
              </div>
              <button onClick={handleNewQuestion}
                className="text-sm text-primary border border-primary/30 px-5 py-2 rounded-xl hover:bg-primary/5 transition-colors font-mono">
                Try again
              </button>
            </div>
          )}
